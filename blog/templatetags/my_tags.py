...
@register.inclusion_tag("F:/play/cnblog/templates/left_region.html")
def get_query_data(username):
    ...
    # 日期归档
    date_list = Article.objects.filter(user=user).extra(select={"y_m_date": "date_format(create_time,'%%Y/%%m')"}).values(
